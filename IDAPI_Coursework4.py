...
def sample_beta_0(y, x, beta_1, beta_2, tau, mu_0, tau_0):
    ...
    mean = (mu_0*tau_0 + tau*np.sum(y-beta_1*x - beta_2 * x**2))/precision
...
def sample_beta_1(y, x, beta_0, beta_2, tau, mu_1, tau_1):
    ...
    mean = (tau_1 * mu_1 + tau * np.sum((y - beta_0-beta_2*x**2) * x))/precision
...
def sample_beta_2(y, x, beta_0, beta_1, tau, mu_2, tau_2):
    ...
    precision = tau_2 + tau * np.sum(x**4)
    mean = (tau_2 * mu_2 + tau * np.sum((y - beta_0-beta_1*x) * x**2))/precision
    return np.random.normal(mean, 1 / np.sqrt(precision))
...
def sample_tau(y, x, beta_0, beta_1, beta_2, alpha, beta):
    ...
    alpha_new = alpha + N / 2
    remainder = y - beta_0 - beta_1 * x - beta_2 * x**2
    beta_new = beta + np.sum(remainder* remainder) / 2
...
if __name__ == '__main__':
    print('IDAPI Sampling Coursework')
    ##########################
